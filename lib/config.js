...
import reserved from '../config/reserved.js'
...
const merge = (...args) => Object.assign(...args.filter(Boolean).map(dcopy))
...
const format = {
  oauth: ({ oauth }) => parseInt(oauth) || undefined,
  key: ({ oauth, key, consumer_key, client_id }) =>
    oauth === 1 ? key || consumer_key : oauth === 2 ? key || client_id : undefined,
  secret: ({ oauth, secret, consumer_secret, client_secret }) =>
    oauth === 1 ? secret || consumer_secret : oauth === 2 ? secret || client_secret : undefined,
  ...
  custom_params: (provider) => {
    let params = provider.custom_params || {}
  ...
  overrides: (provider) => {
    let overrides = provider.overrides || {}
    ...
    overrides = Object.keys(overrides).reduce(
      (all, key) => ((all[key] = init(provider, overrides[key])), all),
      {}
    )
...
const state = (provider, key = 'state', value = provider[key]) =>
...
const pkce = (code_verifier = crypto.randomBytes(40).toString('hex')) => ({
  code_verifier,
...
const transform = (provider) => {
  Object.keys(format).forEach((key) => (provider[key] = format[key](provider)))

  // filter undefined
  return dcopy(provider)
}

const init = compose(merge, filter, transform)

const compat = (config) =>
  config.fitbit2
    ? Object.assign({}, config, { fitbit2: Object.assign({}, oauth.fitbit, profile.fitbit, config.fitbit2) })
    : config.linkedin2
    ? Object.assign({}, config, {
        linkedin2: Object.assign({}, oauth.linkedin, profile.linkedin, config.linkedin2)
      })
    : config.zeit
    ? Object.assign({}, config, { zeit: Object.assign({}, oauth.vercel, profile.vercel, config.zeit) })
    : config

const defaults = ({ path, prefix = '/connect', ...rest } = {}) => ({
  ...rest,
...
const ctor = (
  (_defaults) =>
  (config = {}, defaults = _defaults(config.defaults)) =>
    Object.keys(compat(config))
      .filter((name) => !/defaults/.test(name))
      .reduce(
        (all, name) => (
          (all[name] = init(oauth[name], profile[name], defaults, config[name], { name, [name]: true })), all
        ),
        { defaults }
      )
)(defaults)
...
const provider = (config, session, _state = {}) => {
  const name = session.provider
  let provider = config[name]
  ...
  if (session.override && provider.overrides) {
    const override = provider.overrides[session.override]
    if (override) {
      provider = override
    }
  }
  ...
  if (provider.pkce) {
    provider = dcopy(provider)
    ;({ code_verifier: provider.code_verifier, code_challenge: provider.code_challenge } = pkce())
  }

  return provider
}

export default Object.assign(ctor, {
  compose,
  dcopy,
  merge,
  filter,
