...
module.exports =
  ...
  ({ options, options: { headers }, body }) => {
    var header = Object.keys(options.headers).find((name) => name.toLowerCase() === 'content-type')
    ...
    var uri = url.parse(
      `${options.protocol}//${options.hostname}${options.path}` + (options.port ? `:${options.port}` : '')
    )

    var result = buildOauth({
