...
export const authorize = async ({ provider, input }) => {
...
export const access =
  ({ request: client }) =>
  async ({ provider, input, input: { query, body, session } }) => {
    query = Object.keys(query).length ? query : body
    if (!query.code) {
      const output = Object.keys(query).length ? query : { error: 'Grant: OAuth2 missing code parameter' }
      return { provider, input, output }
    } else if (session.state && query.state !== session.state) {
      const output = { error: 'Grant: OAuth2 state mismatch' }
      return { provider, input, output }
    }
    const options = {
      method: 'POST',
    ...
    if (
      /autodesk|ebay|fitbit|homeaway|hootsuite|notion|reddit|trustpilot/.test(provider.name) ||
    ...
    if (provider.token_endpoint_auth_method === 'private_key_jwt') {
      ...
      const assertion = (async () => {
        const { public_key, private_key } = provider
        return await oidc.sign(
          jwt({
            kid: private_key.kty ? await oidc.kid(private_key) : undefined,
            x5t: public_key ? (public_key.kty ? public_key.x5t : await oidc.x5t(public_key)) : undefined,
            secret: private_key.kty ? await oidc.pem(private_key) : private_key
          })
        )
      })()

      options.form.client_assertion_type = 'urn:ietf:params:oauth:client-assertion-type:jwt-bearer'
      options.form.client_assertion = await assertion
      delete options.form.client_id
      delete options.form.client_secret
    }
    if (provider.instagram && /^\d+$/.test(provider.key)) {
    ...
    if (provider.smartsheet) {
      delete options.form.client_secret
      const hash = await hashWithWebCrypto('SHA-256', provider.secret + '|' + query.code)
      options.form.hash = hash
    }
