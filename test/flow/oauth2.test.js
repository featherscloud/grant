...
describe('oauth2', () => {
  ...
  describe('custom', () => {
    ...
    it('access - basic auth', async () => {
      ...
      await Promise.all(
        ['autodesk', 'ebay', 'fitbit', 'homeaway', 'hootsuite', 'notion', 'reddit', 'trustpilot'].map(
          (provider) =>
            request({
        ...
        responses.forEach(({ body: { response } }) => {
          t.deepEqual(response, {
            access_token: 'token',
            refresh_token: 'refresh',
            raw: { access_token: 'token', refresh_token: 'refresh', expires_in: '3600' }
