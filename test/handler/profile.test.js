    ...
    for (const name of providers) {
      ...
      provider[`oauth${version}`].on.profile = ({ method, query, headers, form }) => {
        'arcgis' === name
          ...
          : 'constantcontact' === name
          ? t.equal(query.api_key, 'key')
          : 'baidu' === name
          ? t.equal(query.access_token, 'token')
          : 'deezer' === name
          ? t.equal(query.access_token, 'token')
          : 'disqus' === name
          ? t.equal(query.api_key, 'key')
          ...
          ? t.equal(method, 'POST')
          : 'echosign' === name
          ? t.equal(headers['access-token'], 'token')
          : 'flickr' === name
          ? t.deepEqual(query, { method: 'flickr.urls.getUserProfile', api_key: 'key', format: 'json' })
          : 'foursquare' === name
          ? t.equal(query.oauth_token, 'token')
          : 'getpocket' === name
          ? t.deepEqual(query, { consumer_key: 'key', access_token: 'token' })
          : 'instagram' === name
          ? t.equal(query.access_token, 'token')
          : 'mailchimp' === name
          ? t.equal(query.apikey, 'token')
          : 'meetup' === name
          ? t.equal(query.member_id, 'self')
          : 'mixcloud' === name
          ? t.equal(query.access_token, 'token')
          : 'shopify' === name
          ? t.equal(headers['x-shopify-access-token'], 'token')
          : 'slack' === name
          ? t.equal(query.token, 'token')
          : 'soundcloud' === name
          ? t.equal(query.oauth_token, 'token')
          : 'stackexchange' === name
          ? t.equal(query.key, 'token')
          : 'stocktwits' === name
          ? t.equal(query.access_token, 'token')
          : 'tiktok' === name
          ? (t.equal(method, 'POST'),
          ...
          : 'tumblr' === name
          ? t.equal(query.api_key, 'token')
          : 'vk' === name
          ? t.deepEqual(query, { access_token: 'token', v: '5.103' })
          : 'wechat' === name
          ? t.deepEqual(query, { access_token: 'token', openid: 'openid', lang: 'zh_CN' })
          : 'weibo' === name
          ? t.deepEqual(query, { access_token: 'token', uid: 'id' })
          : 'twitter' === name
